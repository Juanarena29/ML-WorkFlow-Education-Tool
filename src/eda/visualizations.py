...
Cada funcion recibe datos como parametro y devuelve una figura Plotly
sin depender de Streamlit ni del estado global.
"""
...
def distribution_categorical_fig(
    df: pd.DataFrame,
    column: str,
) -> go.Figure:
...
def relations_scatter_fig(
    ...
    return px.scatter(
        df, x=x_col, y=y_col, color=color_col, title=f"{x_col} vs {y_col}"
    )


def target_distribution_fig(
    df: pd.DataFrame,
    target_column: str,
    problem_type: str,
) -> go.Figure:
    ...
    if problem_type == "classification":
        ...
        return px.bar(
            counts, x=target_column, y="conteo",
            title=f"Distribucion de {target_column}",
        )

    return px.histogram(
        df, x=target_column, nbins=30,
        title=f"Distribucion de {target_column}",
    )


def target_relation_fig(
    df: pd.DataFrame,
    target_column: str,
    feature_column: str,
    feature_type: str,
